...
module.exports = ctx => {
  ...
  return function imgTag(args) {
    ...
    if (src.endsWith('.svg')) {
      // <object type="image/svg+xml" data="{% asset_path loan.svg %}"></object>
      const attrs = {
        type: 'image/svg+xml',
        data: src,
        // class: classes.join(' '),
      ...
      const tag = htmlTag('object', attrs, '');
      const div = htmlTag('div', {}, tag, false);
      return htmlTag('a', { href: src, class: 'gallery-item' }, div, false);
    }
